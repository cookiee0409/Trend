...
// 네이버 뉴스 검색 mock
function naverNews(keyword, n) {
  n = n || 5;
  var templates = [
    { t: "{k} 관련 서비스 출시 발표", d: "{k} 관련 새 서비스가 출시됐다고 발표했다." },
    { t: "{k} 인상 논란 확산", d: "{k} 인상을 두고 논란이 일고 있다." },
    { t: "{k} 정부 대책 확정", d: "정부가 {k} 관련 대책을 확정했다." },
    { t: "{k}, 업계 반응은", d: "{k}에 대한 업계의 반응을 정리했다." },
    { t: "{k} 전망 분석", d: "{k}의 향후 전망을 분석했다." }
  ];
  var out = [];
  for (var i = 0; i < n; i++) {
    var tp = templates[i % templates.length];
    out.push({
      title: tp.t.replace(/\{k\}/g, keyword),
      description: tp.d.replace(/\{k\}/g, keyword),
      link: "https://n.news.naver.com/mock/" + encodeURIComponent(keyword) + "/" + i,
      originallink: "https://news.example.com/" + encodeURIComponent(keyword) + "/" + i,
      pubDate: new Date(Date.now() - i * 3600000).toUTCString()
    });
  }
  return out;
}
...
module.exports = { googleTrends, trends24, naverDatalab, naverBlog, naverCafe, naverNews, youtubePopular, youtubeSearch };
