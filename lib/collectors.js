...
/* ---------- 네이버 검색 API 공용 ---------- */
// 검색(블로그/카페)은 데이터랩과 별도의 앱/키를 쓸 수 있다.
// NAVER_SEARCH_CLIENT_ID/SECRET 가 있으면 사용하고, 없으면 기존 NAVER_CLIENT_ID/SECRET 로 폴백.
function searchCreds() {
  return {
    id: process.env.NAVER_SEARCH_CLIENT_ID || process.env.NAVER_CLIENT_ID,
    secret: process.env.NAVER_SEARCH_CLIENT_SECRET || process.env.NAVER_CLIENT_SECRET
  };
}
...
async function naverBlog(keyword, opts) {
  const cred = searchCreds();
  const id = cred.id, secret = cred.secret;
  if (!id || !secret) {
...
/* ---------- 네이버 뉴스 검색 ---------- */
async function naverNews(keyword, opts) {
  const cred = searchCreds();
  const id = cred.id, secret = cred.secret;
  if (!id || !secret) {
    return { ok: true, usedMock: true, items: mock.naverNews(keyword, 5), message: "뉴스: API 키 없음 → mock" };
  }
  try {
    const raw = await naverSearch("news", keyword, opts);
    const items = raw.map(function (it) {
      return {
        title: stripHtml(it.title),
        description: stripHtml(it.description),
    ...
    return { ok: true, usedMock: false, items: items, message: "뉴스 검색 성공 (" + items.length + "건)" };
  } catch (e) {
    return { ok: true, usedMock: true, items: mock.naverNews(keyword, 5), message: "뉴스 검색 실패 → mock (" + e.message + ")" };
  }
}

module.exports = { googleTrends, trends24, naverDatalab, naverBlog, naverCafe, naverNews };
