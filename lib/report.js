...
async function generate(type) {
  ...
  // 인기 영상: video_id 별 최신 스냅샷(popular) 1개
  const popLatest = {};
  ytSnaps.filter(function (s) { return s.source_type === "popular"; }).forEach(function (s) {
  ...
  const popVideos = Object.keys(popLatest).map(function (vid) {
    const s = popLatest[vid]; const v = ytVideoMap[vid] || {};
    return { video_id: vid, title: v.title || "", channel_title: v.channel_title || "", category_id: v.category_id || "",
  ...
  // 키워드 연결: 기간 내 yt_keyword 관계
  const ytKwRows = (await store.getYtKeyword()).filter(function (r) { const d = kstDateStr(r.collected_at); return d >= range.start && d <= range.end; });
  const ytKwMap = {};
