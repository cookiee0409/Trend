...
function naverRising(naverRows, limit) {
  ...
  Object.keys(byGroup).forEach(function (g) {
    ...
    out.push({ group: g, latest: latest, deltaPrev: latest - prev, deltaAvg: Math.round((latest - avg) * 10) / 10 });
  });
...
async function generate(type) {
  ...
  const naverInRange = allNaver.filter(function (r) { return r.date >= range.start && r.date <= range.end; });
  ...
  md += "- 수집 스냅샷 수: " + snaps.length + "건 / 네이버 데이터랩 " + naverInRange.length + "행\n\n";
  ...
  const newCands = candidates.filter(function (c) { return kstDateStr(c.first_seen_at) >= range.start; }).slice(0, 15);
  md += "## 5. 새로 발견된 키워드 후보\n";
