...
async function generate(type) {
  ...
  md += "---\n";
  md += "_데이터 소스: Google Trends(RSS), Trends24(제3자 사이트), 네이버 데이터랩/블로그/카페 검색._\n";
  md += "_네이버 블로그·카페 검색 결과는 전수 데이터가 아니며, 신규 글 수는 내부 수집 기준의 참고 지표입니다._\n";
  ...
  return await store.addReport({
    ...
    summary: summary,
    content_markdown: md,
    content_json: JSON.stringify({ type: type, snapshots: snaps.length, blog: blogPosts.length, cafe: cafePosts.length })
  });
}
